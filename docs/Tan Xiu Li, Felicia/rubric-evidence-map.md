...
| CRUD | Evidence |
|------|----------|
| **Create** | Manual create Tenant/Staff where allowed (`POST /user/manual-create`); face enrolment `POST /user/enroll-face` (self, or FM via `targetUserId`); attendance + security logs generated by recognition (`POST /api/attendance/scan`, `POST /api/security/logs`). |
