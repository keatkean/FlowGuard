...
- As of 2026-07-12: scanners run TWO loops — detection-only `POST /api/facial-recognition/track` (FastAPI `/user/track`, ~250 ms, box + headTurnRatio, no identity/DB) and full `/recognize` (~1 s, only in TARGET_LOCKING). Liveness = baseline-delta head-turn (`client/src/constants/liveness.js`), access granted only after a final same-ID `/recognize`. Pi/AI Python tests run with `ai-service/.venv/Scripts/python -m pytest` (pytest, httpx, flask were pip-installed into that venv as test-only deps — NOT in requirements.txt).
